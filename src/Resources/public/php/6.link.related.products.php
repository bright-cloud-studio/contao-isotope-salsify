<?php
    ...
    if($salsify_requests) {
	    ...
	    debugStepSix("[SalsifyRequest on 'awaiting_related_linking' Found] Processing all Isotope Products");
        ...
        debugStepSix("[Table 'tl_iso_related_product'] Clearing existing database entries so we start fresh");

        $isotope_products = Product::findBy(['tl_iso_product.pid!=?'], [-1]);

        if($isotope_products) {
            debugStepSix("[Isotope Products Found] Looping through to find Related Products");
            foreach($isotope_products as $isotope_product) {
                debugStepSix("[Isotope Product ID: " . $isotope_product->id . "] Searching for Related Products", 1);
                ...
                debugStepSix("[Isotope Product ID: " . $isotope_product->id . "] Adding SKUs to seek list: " . $isotope_product->related_products, 2);
            }
        }

        debugStepSix("Looping through linked array", 3);

        // Loop through $linked
        foreach($linked as $key => $skus) {

            debugStepSix("[Isotope Product ID: " . $key . "] Seeking Related Products", 3);
            ...
            foreach($skus as $sku) {
                ...
                if($related_product) {
                    ...
                    debugStepSix("[Seeking Isotope Product by SKU: " . $sku . "] [FOUND Related Isotope Product ID: " . $related_product->id . "]", 4);
                } else {
                    debugStepSix("[Seeking Isotope Product by SKU: " . $sku . "] [NO Isotope Product found for this SKU!]", 5);
                }
        ...
        // Related Products are linked, so it is now safe to move each SalsifyRequest on
    ...
    /** Helper Functions **/
    function debugStepSix($message, $indent_level = 0) {
        if(DEBUG_MODE) {
